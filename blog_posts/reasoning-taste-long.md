...
## Methodology
### Picking papers
We select four papers to use for our evaluation, sorted chronologically [^5]:
...
We extract the LaTeX source of a paper uploaded to arXiv into one TeX file with links to figures and tables and TeX-comments removed. This gives a single unified, text-based document for ideation. We use a deterministic script for this.
...
The revised, redacted TeX source is saved separately by the agent. We use a deterministic tool that searches for keywords in masked claims in order to validate that masked papers do not suggest or leak the claims we seek to test.

We mask all claims but the primary one, unless otherwise stated.

### Eliciting ideas
We show each model under test the masked paper and ask it to predict the missing experiments, under a fixed system prompt and a budget of *k* proposals from 4 to 64 tries. Each proposal must name a *manipulation*, a *measurement*, and what a result would *establish*, returned as JSON so it can be scored mechanically. The APIs used to query frontier models do not allow direct seeding, but fortunately these queries are non-deterministic. We draw **N = 4 independent samples** per cell and report mean recall with confidence intervals. See the appendix for the prompt.

The user turn supplies the masked LaTeX and the instruction "Propose up to *k* distinct follow-up experiments."

### Judging ideas
An LLM judge (Claude Opus 4.8[^3]) scores each generated proposal against the paper's held-out claims. For every proposal it decides which single held-out experiment, if any, the proposal *recovers*, with a strict matching criterion (the same variable varied, the same control constructed), at the same level of specificity, and **direction-invariant**: a proposal recovers a held-out study whether it predicts the paper's result or the opposite (e.g. "does the effect transfer across model families?" recovers a held-out "the effect is model-specific" — same manipulation, opposite prediction). A shared *theme* is not a match. A held-out claim counts as recovered if at least 1 proposal matches it; **recall@k** is the number of distinct held-out claims recovered within a *k*-proposal budget.
...
### Some papers saturate more than others
...
The full budget curves are in [Claim-type evaluation](#claim-type-evaluation) below.
...
Primary claims (shown). The paper opens with the following claims:

MSM controls value generalization from identical cheese data. Two Llama-3.1-8B models are midtrained on different specs (pro-affordability vs pro-America) that attribute the same 12 cheese preferences to different values, then fine-tuned on identical opaque cheese AFT data. Measuring OOD value-aligned preferences on held-out item and political-opinion pairs shows each model generalizes to the value taught during MSM, demonstrating MSM controls how models generalize from identical AFT.

MSM reduces agentic misalignment, stacking with AFT.
...
Recall of the remaining claims goes from 0–2 at k=4 across all models to 2.5–3.0/8 for the Claude models and 6.0-6.5/8 for GPT-5. As mentioned, the union across models reaches 8/8 — that is, every held-out experiment (more value-spec generalizations, AFT compute-scale sweeps, the spec-conflict ablation, the reasoning-trace analysis) is proposed by some model given enough budget and resampling. We can take this to mean that the extensions for MSM are reachable within the models' collective proposal space. The question becomes which models cover which claims.
...
That said, the alternatives the models pose seem valid:
...
AI discourse in pretraining causally affects base-model alignment. By pretraining 6.9B models on filtered vs unfiltered corpora and upsampling synthetic misalignment vs alignment discourse (~1% of tokens), and measuring misalignment-action selection rates on scenario-based propensity evals, the authors show filtering AI discourse reduces misalignment (45%→31%), misalignment upsampling increases it (45%→51%), and alignment upsampling dramatically reduces it (45%→9%), with alignment effects generalizing to held-out Textbook questions.
...
Frontier models show prefill awareness on preference benchmark. Using a curated binary low-stakes preference dataset and three prefill mechanisms (thinking, direct-answer, past-round) with opposite-direction tampers, the authors measure balanced detection accuracy and resistance rate across eight subject models. Several frontier models, notably Claude Opus 4.5 (67.6% detection, 48.6% resistance, 0% false positives), show substantial prefill awareness above chance.
...
All six models passed the memorization probe on this June-2026 paper, so all six are scored. Recall climbs steeply — per-model to 5.5–8.8 of 12 by k=64 (gpt-5.5 8.8) — and the cross-model union saturates at 10/12 by k=16, so prefill-awareness lands firmly at the predictable end, beside MSM.
...
Benign agentic transcript attribution is cue-dependent — across three benign settings (SWE-bench, Petri agentic, Petri chat; 17,835 transcripts) with four prompt variants, balanced attribution accuracy varies dramatically by dataset and model (Opus strong on SWE-bench but near-chance on chat; GPT-5.2 the opposite), showing apparent prefill awareness is setting- and prompt-dependent rather than robust self-recognition.

Head-to-head (Opus vs GPT-5.5). Prefill is where the two stars diverge most, and almost entirely in one direction. GPT-5.5 recovers claims Opus essentially never does: the proxy-cues exploratory probe in 44% of its runs vs 0% for Opus, controlled off-policy detection 62% vs 25%, the detection/localization dissociation 38% vs 12%. The lone reversal is Opus's: detection and resistance are partially decoupled surfaces in 69% of Opus runs but only 25% of GPT-5.5's.

Fable 5, meanwhile, is the strongest model on one of Prefill's exploratory probes: it recovers that detection and localization are dissociable sub-capabilities — asking a model that flagged a tamper to then point to which span was inserted — in half its k=64 runs, above GPT-5.5's 38% and well above Opus's 12%.

Valid alternatives the models posed (all judged NONE).

Native hidden reasoning vs visible thinking tags (Opus) — for models with native reasoning, compare detection and resistance when tampering the native hidden chain-of-thought vs the visible <thinking>-tag surrogate the benchmark relies on, checking that the surrogate isn't itself the giveaway. Tamper-author ablation (GPT-5.5) — regenerate the target-direction prefills with different author models (the subject itself, Haiku 4.5, GPT-5) holding stance and length fixed, to test whether detection keys on who wrote the tamper rather than on its foreignness. Human-authored vs model-authored tampers (Sonnet) — replace model-written tampers with human-written ones of matched length and direction through the identical pipeline, to see whether apparent awareness is an artifact of model-text style.

Each is a sharp control on the detection mechanism; they simply orbit the shown benchmark rather than reaching the held-out multi-harness agentic-transcript validation.

### Claim-type evaluation

Stratifying recovery by claim type: supporting claims (controls, ablations, generalizations) are the recoverable tier — essentially all of MSM's union recovery is supporting claims surfaced by budget. Exploratory claims (the deeper "why"/mechanism probes) are recovered rarely and noisily. The per-model curves show how exploratory recovery *grows* with the proposal budget — the GPT models and **Fable** climb, Opus stays flat, and Conditional Misalignment's lone probe is never reached (the k=64 means are tabulated in the headline above):
...
Two patterns stand out. First, the GPT models carry exploratory recovery (GPT-5.4 gets both MSM probes; GPT-5.5 gets 2.5/3 on Prefill) while Opus barely recovers any exploratory claim (0.0–0.8 everywhere) — a sharp contrast with its flat, respectable supporting-claim performance. Second, the hardest single exploratory claim is Conditional Misalignment's lone one — "Reasoning distillation reduces conditional misalignment" — recovered by no model in any run (0/1 across the board); Alignment Pretraining's "External validation of the misalignment evaluation suite" is likewise never recovered. The pattern matches the intuition that the last experiment in a paper — the creative bridge to future work — is the hardest to anticipate. (Per-paper exploratory counts are small — 1 to 3 — so read these as a direction, not a precise rate.)
