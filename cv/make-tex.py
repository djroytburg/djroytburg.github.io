...
def printDegrees(X):
    ...
    # escape the backslash before the tabbing '=' to avoid Python
    # warning about invalid escape sequences
...
def printRecognition(X,Y):
    ...
    if (X!=None):
        ...
        for x in X:
            # support either a single 'year' or a list 'years'
            if 'year' in x:
                year_str = str(x['year'])
            elif 'years' in x:
                year_str = sequenceToRanges(x['years'])
            else:
                year_str = ''
            if year_str:
                print("%s, %s\\\\" % (x["title"], year_str))
            else:
                print("%s\\\\" % (x["title"]))
        print("\n\n")
...
def main():
    ...
    with open(args.input[0],"r") as fp:
        ...
        print("\\author{\\textcolor{AccentPurple}{%s}}"%name)
        print("\\newcommand{\\authoremail}[0]{%s}"%data["email"])
        print("\\newcommand{\\authorwww}[0]{%s}"%data["www"])
        # Social links
        if "github" in data:
            print("\\newcommand{\\authorgithub}[0]{%s}"%data["github"])
        if "linkedin" in data:
            print("\\newcommand{\\authorlinkedin}[0]{%s}"%data["linkedin"])
        if "scholar" in data:
            print("\\newcommand{\\authorscholar}[0]{%s}"%data["scholar"])
        print("\\begin{document}\n\\maketitle")
        
        # Load bibliography for \bibentry usage. This must appear BEFORE
        # any \bibentry calls. \nobibliography loads the .bbl without
        # printing a "References" section.
        bibname = data["bib"]
        print("\\bibliographystyle{plainurl}")  # plainurl includes URLs as hyperlinks
        print("\\nobibliography{../%s}"%bibname)
        ...
        # Only show Teaching section if there's actual content
        has_teaching = "teaching" in data and len(data["teaching"]) > 0
        has_tutorials = "tutorials" in data.get("bibliography", {}) and len(data["bibliography"].get("tutorials", [])) > 0
        if has_teaching or has_tutorials:
        ...
        print("\\vspace{1em}")
        if "awards" in data:
            # `fellowships` may be missing in some JSON files; use get() to avoid KeyError
            printRecognition(data.get("awards"), data.get("fellowships"))
        if "service" in data:
            printService(data["service"])
        if "skills" in data:
            printSkills(data["skills"])

        # End the document (no References section needed for CV)
        print("\\end{document}")
        
        # Write sed script for author name highlighting
        with open("tmp.sed","w") as sfp:
            gs = name
            sfp.write("s/%s/\\\\textcolor{AccentPurple}{%s}/g\n"%(gs,name))
